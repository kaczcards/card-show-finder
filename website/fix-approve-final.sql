-- ================================================================
-- FINAL FIX: approve_pending_show Function
-- ================================================================
-- Removed ON CONFLICT clause since there's no unique constraint
-- ================================================================
...
BEGIN
  ...
  -- Insert into shows table (NO ON CONFLICT)
  BEGIN
  ...
  -- Try to log the action (optional, won't fail if table doesn't exist)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Silently ignore if admin_feedback table doesn't exist
    NULL;
  END;
...
COMMENT ON FUNCTION public.approve_pending_show IS 
  'Approves a pending show and publishes it to the shows table (updated to remove ON CONFLICT)';
