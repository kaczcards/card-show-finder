#!/usr/bin/env node
/**
 * scraper/run-csv-batch.js
 * 
 * Script to run the scraper on URLs from the "show list - Sheet1.csv" file.
 * Processes URLs in batches with delays between them.
 * Can filter by state if needed.
 * 
 * Usage:
 *   node scraper/run-csv-batch.js [options]
 *   
 * Options:
 *   --state STATE    Filter URLs by state (2-letter code, e.g., TX)
 *   --batch SIZE     Number of URLs to process in each batch (default: 5)
 *   --delay MS       Delay between URL processing in ms (default: 2000)
...
const CSV_FILE_PATH = path.join(__dirname, 'show list - Sheet1.csv');

// Parse command line arguments
const argv = minimist(process.argv.slice(2), {
  string: ['state', 'batch', 'delay'],
  boolean: ['help'],
  alias: {
    h: 'help',
    s: 'state',
...
Process URLs from "show list - Sheet1.csv" and extract card show information.
...
Options:
  ...
  -b, --batch SIZE    Number of URLs to process in each batch (default: 5)
  -d, --delay MS      Delay between URL processing in ms (default: 2000)
  -h, --help          Show this help text

Examples:
  # Process all URLs from CSV
  node scraper/run-csv-batch.js

  # Process only Indiana URLs
  node scraper/run-csv-batch.js --state IN

  # Process in larger batches with longer delays
  node scraper/run-csv-batch.js --batch 10 --delay 5000
  `);
}
...
function loadUrlsFromCsv() {
  try {
    if (!fs.existsSync(CSV_FILE_PATH)) {
      logError('CSV file not found', `Expected at: ${CSV_FILE_PATH}`);
      process.exit(1);
    }
    ...
    const urlList = records.map(record => ({
      state: record.State?.trim() || '',
      url: record.URL?.trim() || '',
      frequency: record.Frequency?.trim() || '',
      notes: record[Object.keys(record)[3]]?.trim() || '' // Get the 4th column (Notes)
    })).filter(item => item.url && item.state); // Filter out entries without URL or state
    
    log(`Loaded ${urlList.length} URLs from CSV file`);
    return urlList;
  } catch (error) {
    logError('Failed to load URLs from CSV', error);
    process.exit(1);
  }
}

// Filter URLs by state
function filterUrlsByState(urlList, stateFilter) {
  if (!stateFilter) return urlList;
...
async function processUrl(url, state, notes, supabase) {
  log(`Processing URL: ${url} (State: ${state}${notes ? ', Notes: ' + notes : ''})`);
  ...
  try {
    ...
    for (const show of allShows) {
      ...
      const rawPayload = {
        ...
        city: show.city || null,
        state: show.state || state || null, // Use the state from CSV if not in the show data
        entryFee: show.entryFee || null,
        description: show.description || null,
        url: show.url || url,
        contactInfo: show.contactInfo || null,
        extractedAt: new Date().toISOString(),
        sourceNotes: notes || null // Add the notes from CSV
      };
...
async function processBatches(urlsToProcess, batchSize, delayMs, supabase) {
  const results = [];
  ...
  log(`Processing ${urlsToProcess.length} URLs in ${batches.length} batches of up to ${batchSize} URLs each`);
  
  // Process each batch
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    const batch = batches[batchIndex];
    log(`Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} URLs)`);
    
    // Process URLs in the batch
    for (const item of batch) {
      const { url, state, notes } = item;
      const result = await processUrl(url, state, notes, supabase);
      results.push({ url, state, ...result });
    ...
    if (batchIndex < batches.length - 1) {
      log(`Batch ${batchIndex + 1} complete. Waiting for ${delayMs * 2}ms before starting next batch...`);
...
async function main() {
  ...
  // Parse batch size and delay
  const batchSize = parseInt(argv.batch, 10) || 5;
  const delayMs = parseInt(argv.delay, 10) || 2000;
  ...
  // Load URLs from CSV
  const allUrls = loadUrlsFromCsv();
  
  // Filter by state if needed
  const urlsToProcess = argv.state 
    ? filterUrlsByState(allUrls, argv.state)
    : allUrls;
  ...
  const { 
    ...
    successfulScrapes, 
    failedScrapes 
  } = await processBatches(urlsToProcess, batchSize, delayMs, supabase);
  
  const elapsedSeconds = ((Date.now() - startTime) / 1000).toFixed(2);
  const elapsedMinutes = (parseFloat(elapsedSeconds) / 60).toFixed(2);
  
  // Print summary
  log(`CSV Batch Scraper completed in ${elapsedSeconds}s (${elapsedMinutes} minutes)`, {
    totalUrls: urlsToProcess.length,
    batchSize,
    delayBetweenUrls: `${delayMs}ms`,
    successfulScrapes,
    failedScrapes,
    totalShowsFound,
    stateFilter: argv.state ? argv.state.toUpperCase() : 'None',
    results: results.map(r => ({
      url: r.url,
      state: r.state,
      success: r.success,
      showsFound: r.showCount
    }))
  });
  ...
  for (const result of results) {
    ...
    if (!stateStats[state]) {
      stateStats[state] = { urls: 0, successful: 0, failed: 0, shows: 0 };
    }
    stateStats[state].urls++;
    if (result.success) {
      stateStats[state].successful++;
      stateStats[state].shows += result.showCount;
    } else {
      stateStats[state].failed++;
    }
  }
  
  log('Results by State:', stateStats);
}
