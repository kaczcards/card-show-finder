...
CREATE POLICY "want_lists_select_mvp_dealer"
  ...
  USING (
    ...
    EXISTS (
      ...
      JOIN shows s ON swl.show_id = s.id
      WHERE swl.want_list_id = want_lists.id
      AND (
...
CREATE POLICY "shared_want_lists_select_self"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = shared_want_lists.want_list_id
      AND userid = auth.uid()
    )
  );
...
CREATE POLICY "shared_want_lists_select_mvp_dealer"
  ...
  USING (
    ...
    (
      ...
      EXISTS (
        SELECT 1 FROM shows
        WHERE id = shared_want_lists.show_id
        AND organizer_id = auth.uid()
      ) OR
      -- Or they participate in the show (using safe function)
      participates_in_show_safe(shared_want_lists.show_id)
    )
  );
