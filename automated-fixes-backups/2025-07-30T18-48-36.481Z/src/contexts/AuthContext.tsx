...
console.warn('[_AuthContext] Fetching user profile from database...');
  ...
  const _addFavoriteShow = async (showId: string): Promise<void> => {
    try {
      if (!authState.user) {
        throw new Error('User not authenticated');
      }
...
console.warn('[_AuthContext] Show added to favorites successfully');
  ...
  const _removeFavoriteShow = async (showId: string): Promise<void> => {
    try {
      if (!authState.user) {
        throw new Error('User not authenticated');
      }
...
console.warn('[_AuthContext] Show removed from favorites successfully');
   ...
   */
  const _forceRefreshAndFetchProfile = async (userId: string): Promise<User | null> => {
    try {
