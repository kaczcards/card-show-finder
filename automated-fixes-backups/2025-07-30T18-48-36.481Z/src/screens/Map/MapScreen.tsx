...
const MapScreen: React.FC<MapScreenProps> = ({
  ...
  const [filterVisible, setFilterVisible] = useState(_false);
...
console.warn('[_MapScreen] Filters being used:', _currentFilters);
  ...
  const _handleFilterChange = (newFilters: ShowFilters) => {
    if (_onFilterChange) {
  ...
  const _handleRegionChangeComplete = (region: Region) => {
  ...
  const _formatDate = (dateValue: Date | string) => {
    try {
