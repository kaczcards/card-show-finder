...
const _HomeScreen = ({ 
  ...
  const [filterSheetVisible, setFilterSheetVisible] = useState(_false);
  const [presetModalVisible, setPresetModalVisible] = useState(_false);
...
console.warn('App has come to the foreground - refreshing data');
    ...
    if (!isLoading && shows.length === 0 && totalCount > 0 && !useEmergencyList) {
      ...
      const _fetchAllActiveShows = async () => {
        try {
          ...
          console.info(`[_HomeScreen] Emergency fetch found ${data?.length || 0} shows`);
  ...
  const _handleApplyFilters = (newFilters: ShowFilters) => {
    if (_onFilterChange) {
  ...
  const _handleRemoveFilter = (key: string, value?: string) => {
    ...
    if (_onFilterChange) {
      ...
      // Otherwise, update local state
      setLocalFilters(prev => updateFilters(_prev));
    }
  };
...
console.warn(`[_HomeScreen] Client-side filtering: ${shows.length} shows → ${safeShows.length} shows within ${filters.radius} miles`);
  ...
  // Render show item
  const _renderShowItem = ({ item, index }: { item: Show; index: number }) => (
    <TouchableOpacity
