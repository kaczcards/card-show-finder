...
const ProfileScreen: React.FC = () => {
  ...
  const [checkingAdmin, setCheckingAdmin] = useState(_false);
  ...
  const [localFavoriteCount, setLocalFavoriteCount] = useState(_0);
...
console.warn('[_ProfileScreen] Profile updated successfully');
  ...
  const _isDealer = () => {
    ...
    console.debug('[_ProfileScreen] isDealer check', {
      userId: user.id,
      role: user.role,
  ...
  const _getRoleDisplayName = (role: UserRole) => {
