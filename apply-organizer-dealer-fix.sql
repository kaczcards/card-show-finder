-- apply-organizer-dealer-fix.sql
-- Description: Fixes the get_show_details_by_id function to include show organizers
-- when they register as dealers in the participating dealers list.
--
-- This script can be run directly in the Supabase SQL editor to apply the fix
-- without needing to run a full migration. It's safe to run multiple times.
--
-- Problem: Show organizers who register as dealers are not appearing in the 
-- participating dealers list with full MVP privileges because the RPC function
-- only includes users with 'dealer' or 'mvp_dealer' roles.
--
-- Fix: Update the function to also include users with 'show_organizer' role
-- when they register as dealers for a show.
...
BEGIN
  ...
  -- KEY FIX: Added 'show_organizer' to the role filter
  WITH all_dealers AS (
    SELECT
      p.id,
...
-- Confirmation message (will appear in the SQL editor results)
DO $$
BEGIN
  RAISE NOTICE 'Successfully updated get_show_details_by_id function to include show organizers in dealer listings!';
END $$;
