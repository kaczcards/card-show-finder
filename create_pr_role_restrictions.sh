...
echo -e "${BLUE}This script will create a PR for the role-based UI restrictions feature${NC}"
echo
...
if [[ ! -f pr_description_role_based_restrictions.md ]]; then
    echo -e "${RED}PR description file 'pr_description_role_based_restrictions.md' not found!${NC}"
    exit 1
fi
...
gh pr create \
  --base main \
  --title "Role-based UI restrictions and Collection screen restructuring" \
  --body-file pr_description_role_based_restrictions.md \
  --draft
...
echo -e "3. Test the role-based restrictions with different user accounts"
