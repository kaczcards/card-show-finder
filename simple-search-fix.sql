-- SIMPLE SAFE SEARCH: Avoid all array issues by only searching text columns
-- This version focuses on the core functionality and avoids problematic columns
...
BEGIN
  ...
  -- Step 1: Get total count - ONLY search safe text columns
  WITH matching_show_ids AS (
    ...
    WHERE
      ...
      -- Location filter - handle null coordinates
      (
      ...
      -- SAFE KEYWORD SEARCH - Only text columns with null protection
      (
        param_keyword IS NULL OR
        -- Show text fields only
        COALESCE(show_table.title, '') ILIKE '%' || param_keyword || '%' OR
        ...
        -- MVP Dealer booth text fields only
...
'Safe search function that avoids array column issues.
Searches text fields: show title/description/location/address and booth specialty/notable_items/card_types/price_range.
Uses COALESCE to handle nulls safely.';

-- Test with progressive complexity
SELECT 'TEST 1: All active shows in date range' as test_name;
SELECT search_shows_advanced('{
  "start_date": "2025-09-24",
  "end_date": "2025-10-24",
  "page_size": 3
}'::jsonb) as test1_result;

SELECT 'TEST 2: Shows near Carmel, IN' as test_name;
SELECT search_shows_advanced('{
  "lat": 40.0772001,
  "lng": -85.925938,
  "radius_miles": 50,
  ...
  "end_date": "2025-10-24",
  "page_size": 3
}'::jsonb) as test2_result;

SELECT 'TEST 3: Star Wars search' as test_name;  
SELECT search_shows_advanced('{
  "lat": 40.0772001,
  "lng": -85.925938,
  "radius_miles": 50,
