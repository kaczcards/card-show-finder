...
const ShowDetailsScreen = () => {
  ...
  // ---------------------------------------------------------------------------
  //  Show-organizer claim button visibility
  //    - Logged in
  //    - Role is dealer (not showOrganizer yet)
  //    - Show not already claimed
  //    - User is NOT already the promoter
  // ---------------------------------------------------------------------------
  const canClaimShow =
    !!currentUser &&
    userProfile?.role === 'dealer' &&
    userProfile?.role !== 'showOrganizer' &&
    !show?.claimed &&
    !isPromoter;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.detailsContainer}>
        ...
        {/* Claim Show button (dealers only, if unclaimed) */}
        {canClaimShow && (
          <TouchableOpacity
            style={styles.claimButton}
            onPress={() => navigation.navigate('ClaimShow', { showId: show.id })}
          >
            <Ionicons
              name="flag-outline"
              size={18}
...
const styles = StyleSheet.create({
  ...
  claimButton: {
    backgroundColor: '#f39c12',       /* orange to stand out */
    borderRadius: 8,
    height: 48,
    flexDirection: 'row',
    justifyContent: 'center',
