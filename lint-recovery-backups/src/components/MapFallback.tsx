 ...
 * degrade when the native module isn't available (_e.g., in Expo Go).
 * 
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.clusterFallbackText, { marginTop: 10 }, fallbackTextStyle]}>
        {data && Array.isArray(_data) ? `${data.length} item${data.length === 1 ? '' : 's'} would be shown on the map` : 'No data available'}
      </Text>

      {/* ------------------------------------------------------------------
          NEW: Helpful list for Expo Go fallback
         ------------------------------------------------------------------ */}
      {Array.isArray(_data) && data.length > 0 && (
        <ScrollView
