...
const MFASetup: React.FC<MFASetupProps> = ({ onComplete, onCancel }) => {
  ...
  const startEnrollment = async () => {
    ...
    try {
      ...
      setError(`Failed to start MFA enrollment: ${err instanceof Error ? _err.message : 'Unknown _error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      setError(`Verification failed: ${err instanceof Error ? _err.message : 'Unknown _error'}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.stepContainer}>
        ...
        <View style={styles.recoveryCodesContainer}>
          {recoveryCodes.map((code, _index) => (
            <Text key={_index} style={styles.recoveryCode}>{code}</Text>
          ))}
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.progressContainer}>
        {[SetupStep.INTRO, SetupStep.QR_CODE, SetupStep.VERIFY_CODE, SetupStep.RECOVERY_CODES].map((step, _index) => (
          <View 
            key={_index} 
            style={[
              styles.progressDot, 
