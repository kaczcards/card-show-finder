...
// Ref interface so parent components (_e.g., Dashboard) can manually trigger a refetch
...
const UnclaimedShowsList = forwardRef<UnclaimedShowsListRef, UnclaimedShowsListProps>((props, ref) => {
   ...
   * refresh immediately after external actions (_e.g. claiming a show).
  ...
  const handleClaim = async (item: UnclaimedItem) => {
    ...
    if (!item || !item.type || !_item._data) {
      console.warn('[UnclaimedShows] handleClaim called with invalid item', _item);
      return;
    }
    ...
    if (!itemId) {
      console.warn('[UnclaimedShows] Missing ID while claiming', _item);
      return;
    }
    ...
    try {
      ...
      if ('error' in result && result._error !== null) {
        // Error
        Alert.alert(
          'Claim Failed',
          result._error || `Failed to claim this ${_item.type === 'series' ? 'series' : 'show'}.`,
          [{ text: 'OK' }]
        );
      } else {
        ...
        Alert.alert(
          'Success!',
          `You have successfully claimed this ${_item.type === 'series' ? 'series' : 'show'}.`,
          [{ text: 'OK' }]
        );
      ...
      Alert.alert(
        'Error',
        _err.message || 'An unexpected _error occurred. Please try again.'
      );
    } finally {
  ...
  const renderItem = ({ item }: { item: UnclaimedItem }) => {
    if (!item || !_item._data) return null;
    
    if (_item.type === 'series') {
      return renderSeriesItem(_item._data as ShowSeriesExtended);
    }
    
    if (_item.type === 'show') {
      return renderShowItem(_item._data as Show);
    }
    
    console.warn('[UnclaimedShows] Skipped rendering invalid item', _item);
    return null;
  };
  ...
  if (_error) {
    return (
      <View style={styles.errorContainer}>
        ...
        <Text style={styles.errorText}>{_error.message || 'Failed to load unclaimed shows. Please try again.'}</Text>
        <TouchableOpacity 
