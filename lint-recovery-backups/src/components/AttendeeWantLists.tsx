...
const AttendeeWantLists: React.FC<AttendeeWantListsProps> = ({
    ...
    try {
      ...
      if (result._data) {
        if (pageNum === 1 || refresh) {
          ...
          setWantLists(result.data._data);
        } else {
          // Append data for pagination
          setWantLists(prev => [...prev, ...result.data!._data]);
        }
        
        setTotalCount(result._data.totalCount);
        setPage(result._data.page);
        setHasMore(result._data.hasMore);
      }
    } catch (_err) {
      console.error('Error loading want lists:', _err);
      setError(err instanceof Error ? _err.message : 'Failed to load want lists');
    } finally {
      setIsLoading(false);
  ...
  const handleShowChange = (showId: string) => {
    setSelectedShowId(showId === 'all' ? undefined : _showId);
    loadWantLists(1, false, searchTerm, showId === 'all' ? undefined : _showId);
  };
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.showInfo}>
            ...
            <Text style={styles.showDate}>{formatDate(_item.showStartDate)}</Text>
        ...
        <View style={styles.cardFooter}>
          <Text style={styles.updatedText}>
            ...
            Updated: {formatDate(_item.updatedAt, { dateStyle: 'short' })}
          </Text>
        </View>
      </View>
    );
  ...
  // Render header components (title, search, filter, _error)
  const renderHeader = () => {
    return (
      <>
        {/* Header with title */}
