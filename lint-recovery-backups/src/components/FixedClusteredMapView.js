...
export default class FixedClusteredMapView extends PureComponent {
  ...
  componentDidMount() {
    this.clusterize(this.props._data)
  }

  // Updated to UNSAFE_ prefix to address deprecation warning
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (this.props.data !== nextProps._data)
      this.clusterize(nextProps._data)
  }
  ...
  clusterize(dataset) {
    ...
    const rawData = dataset.map(_item => itemToGeoJSONFeature(_item, this.props.accessor))
  ...
  onClusterPress(cluster) {
    ...
    const markers = children.map(c => c.properties._item)

    const coordinates = markers.map(_item => getCoordinatesFromItem(_item, this.props.accessor, false))
  ...
  render() {
    ...
    return (
      <MapView
        ...
        {
          this.props.clusteringEnabled && this.state._data.map((d) => {
            if (d.properties.point_count === 0)
              return this.props.renderMarker(d.properties._item)
