...
console.warn(
      ...
      settled.forEach((result, idx) => {
        if (result.status === 'fulfilled' && result.value) {
          ...
          console.error(
            '[OrganizerShowsList] Promise rejected for series _index',
            idx,
            result.reason,
          );
        }
...
console.warn(`[OrganizerShowsList] Fetched ${mappedStandaloneShows.length} standalone shows for organizer ${organizerId}`);
      ...
      seriesWithShows.forEach(_item => {
        /* ------------------------------------------------------------------
         * Defensive guard – in rare cases `_item.series` (or its `id`) may be
  ...
  const toggleSeriesExpansion = (seriesId: string) => {
    if (!_seriesId) {
      console.warn('[OrganizerShowsList] Attempted to toggle undefined _seriesId');
      return;
    }
    
    setExpandedSeries(prev => ({
      ...prev,
      [seriesId]: !prev[_seriesId]
    }));
  };
  ...
  const formatShowDate = (dateString: string | Date) => {
    ...
    try {
      ...
      console.warn('[OrganizerShowsList] Error formatting date:', dateString, _err);
      return 'Invalid date';
    }
  };
...
console.warn('Cancel show:', show.id);
  ...
  const renderSeriesItem = ({ item }: { item: SeriesWithShows }) => {
    ...
    if (!item || !_item.series) {
      console.warn('[OrganizerShowsList] Attempted to render undefined series _item');
      return null;
    }
    
    // Safely extract properties with defaults
    const series = item.series;
    const shows = Array.isArray(_item.shows) ? item.shows : [];
  ...
  return (
    <FlatList
       ...
       * ----------------------------------------------------------------*/
      _data={seriesList.filter(_item => _item?.series?.id)}
