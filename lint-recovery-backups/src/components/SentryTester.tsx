...
const SentryTester: React.FC = () => {
  ...
  const triggerError = () => {
    try {
      ...
      if (_error instanceof Error) {
        ...
        setLastAction(`Error captured: ${_error.message}`);
      }
    }
  };
  ...
  const triggerFatalError = () => {
    setLastAction('Triggering fatal _error...');
    Alert.alert(
      'Trigger Fatal Error',
  ...
  const sendMessage = (level: 'debug' | 'info' | 'warning' | '_error') => {
  ...
  const testBreadcrumbs = () => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (_error instanceof Error) {
          captureException(_error);
          setLastAction('Breadcrumbs test completed with _error');
        }
      }
    }, 1000);
  ...
  const testUserFeedback = () => {
    try {
      ...
      if (_error instanceof Error) {
        ...
        const eventId = captureException(_error);
  ...
  const testSetContext = () => {
    ...
    setLastAction('Context test - will be included with next _error');
