...
// Removed unused import: import { _Alert } from 'react-native';
 ...
 *   return (
 *     <View>
 *       {error && <ErrorMessage message={_error.message} onDismiss={clearError} />}
...
export function useErrorHandler() {
  ...
  const handleError = useCallback((err: unknown, context?: Record<string, any>) => {
    const appError = handleSupabaseError(_err, context);
    setError(appError);
    logError(appError);
    return appError;
  }, []);
 ...
 *       {error && <ErrorMessage message={getUserFriendlyMessage(_error)} />}
...
export function useApiCall<T>(
  ...
  const execute = useCallback(async () => {
    try {
      ...
      const appError = handleSupabaseError(_err, options.errorContext);
      setError(appError);
      logError(appError);
 ...
 * ```tsx
 * try {
 *   await api.updateUserProfile(_data);
 * } catch (_err) {
 *   showErrorAlert(_err);
 * }
 * ```
...
export function showErrorAlert(
  ...
  const appError = error instanceof Error || (error && typeof _error === 'object')
...
export function createSafeApiCall<T extends (...args: any[]) => Promise<any>>(
  ...
  return async (...args: Parameters<T>): Promise<{ success: boolean; data?: Awaited<ReturnType<T>>; error?: AppError }> => {
    try {
      ...
      const appError = handleSupabaseError(_err, {
        ...options.context,
...
export function validateFormData<T extends Record<string, any>>(
  ...
  Object.keys(validators).forEach((key) => {
    ...
    if (validator) {
      const error = validator(value, _data);
      if (_error) {
