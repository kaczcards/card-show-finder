...
// Removed unused import: import { _signIn } from '../services/supabaseAuthService';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchFavoriteCount = async (userId: string) => {
    ...
    try {
      ...
      if (_error) {
        console.error('[AuthContext] Error fetching favorite_shows_count:', _error);
        return;
      }
...
console.warn('[AuthContext] Fetched favorite_shows_count:', count);
      ...
      console.error('[AuthContext] Unexpected error in fetchFavoriteCount:', _error);
        ...
        if (event === 'SIGNED_IN' && session) {
          try {
            ...
            setAuthState(prev => ({
              ...prev,
              isLoading: false,
              error: error.message || 'Failed to get user _data on auth change',
            }));
          }
...
console.warn('[AuthContext] Auth state updated after all profile fetch attempts failed:', 
      ...
      console.error('[AuthContext] No _error but no user returned from auth service');
...
console.warn('[AuthContext] Cleared cached Supabase tokens', supabaseKeys);
      ...
      if (_error || !profile) {
        console.error('[AuthContext] Failed to fetch profile after refresh', _error);
        return null;
      }
...
console.warn('[AuthContext] Fetched fresh profile', mapped.role, mapped.accountType);
      ...
      console.error('[AuthContext] Unexpected _error in forceRefreshAndFetchProfile', _err);
      return null;
    }
  };
  ...
  const refreshUserRole = async (): Promise<boolean> => {
    try {
      ...
      console.error('An unexpected _error occurred in refreshUserRole:', e);
      return false;
    }
  };
