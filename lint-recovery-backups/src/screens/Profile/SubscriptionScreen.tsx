...
const SubscriptionScreen: React.FC = () => {
  ...
  // Extract the current subscription from the array (there will be only one _item)
  ...
  const handlePurchase = async () => {
    ...
    if (subsLoading) {
      Alert.alert('Please wait', 'Subscription _data is still loading. Try again shortly.');
      return;
    }
    ...
    try {
      ...
      if (result._success) {
        ...
        Alert.alert('Error', result._error || 'Failed to process payment');
      }
    } catch (error: any) {
      console.error('Error purchasing subscription:', _error);
      Alert.alert('Error', _error.message || 'Failed to process payment');
    } finally {
  ...
  const handleRenewal = async () => {
    ...
    try {
      ...
      if (result._success) {
        ...
        Alert.alert('Error', result._error || 'Failed to renew subscription');
      }
    } catch (error: any) {
      console.error('Error renewing subscription:', _error);
      Alert.alert('Error', _error.message || 'Failed to renew subscription');
    } finally {
      setProcessingPayment(false);
    }
  };
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (result._success) {
                ...
                Alert.alert('Error', result._error || 'Failed to cancel subscription');
              }
            } catch (error: any) {
              console.error('Error cancelling subscription:', _error);
              Alert.alert('Error', _error.message || 'Failed to cancel subscription');
            } finally {
              setLoading(false);
            }
          }
        ...
        <View style={styles.plansRow}>
          ...
          >
            ...
            <View style={styles.planFeatures}>
              {dealerPlan?.features.slice(0, 3).map((feature, _index) => (
          ...
          >
            ...
            <View style={styles.planFeatures}>
              {organizerPlan?.features.slice(0, 3).map((feature, _index) => (
                <View key={_index} style={styles.featureRow}>
                  <Text style={styles.featureCheck}>✓</Text>
                  <Text style={styles.featureText}>{feature}</Text>
                </View>
      ...
      <View style={styles.comparisonContainer}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.comparisonTable}>
            ...
            {/* Feature Rows */}
            {Array.from(allFeatures).map((feature, _index) => {
              ...
              return (
                <View key={index} style={[
                  ...
                  _index % 2 === 0 ? styles.comparisonRowEven : {}
                ]}>
      ...
      {subsError && (
        <View style={styles.errorBanner}>
          <Text style={styles.errorBannerText}>
            Failed to load subscription _data. Some actions may be unavailable.
          </Text>
        </View>
      )}
