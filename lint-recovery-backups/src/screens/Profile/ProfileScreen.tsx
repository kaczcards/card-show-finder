...
const ProfileScreen: React.FC = () => {
  ...
  // (_e.g., saveChanges) surfaces its own errors inline.
   ...
   * not exist (_e.g. migration hasn't run yet).
   */
...
console.warn('[ProfileScreen] Fetched favorite_shows_count:', count);
      ...
      console.error('[ProfileScreen] Unexpected _error in fetchFavoriteCount:', _err);
...
console.warn('[ProfileScreen] Screen focused – refreshing counts/badges');
  ...
  const handleLogout = async () => {
    try {
      ...
      Alert.alert('Logout Failed', _error.message || 'Please try again');
    }
  };
...
console.warn('[ProfileScreen] Profile updated successfully');
      ...
      console.error('[ProfileScreen] Error updating profile:', _err);
      Alert.alert('Update Failed', _err.message || 'Please try again');
    } finally {
  ...
  const handleRefreshRole = async () => {
    try {
      ...
      Alert.alert('Error', _e?.message || 'Unexpected _error refreshing session');
    } finally {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      console.error('[ProfileScreen] Error sending password reset:', _error);
      Alert.alert('Password Reset Failed', _error.message || 'Please try again later.');
    } finally {
