...
const geocodeAddress = async (
  ...
  try {
    ...
    console.error('[geocodeAddress] Failed to geocode:', _err);
    throw err;
  }
};
...
const AddShowScreen: React.FC = () => {
  ...
  // Format date & time for display (_e.g. "Wed, Apr 24 2025  10:00 AM")
  ...
  const validateAddress = (): { isValid: boolean; message?: string } => {
    ...
    if (!/^[A-Z]{2}$/.test(stateProv)) {
      return { 
        isValid: false, 
        message: 'State must be a valid 2-letter state code (_e.g., CA, NY, TX)' 
      };
    }
    ...
    if (!/^\d{5}(-\d{4})?$/.test(zipCode)) {
      return { 
        isValid: false, 
        message: 'ZIP code must be 5 digits or 5+4 format (_e.g., 90210 or 90210-1234)' 
      };
    }
  ...
  const validateForm = (): boolean => {
    ...
    if (!stateProv.trim()) {
      ...
      newErrors.stateProv = 'State must be a valid 2-letter code (_e.g., CA)';
    }
...
console.warn('[AddShowScreen] Attempting to geocode address:', fullAddress);
      ...
      if (!coords) {
        ...
        Alert.alert(
          ...
          '• The state code is valid (_e.g., CA, NY, TX)\n' +
...
console.warn('[AddShowScreen] Show created successfully:', _data);
      ...
      console.error('[AddShowScreen] Error creating show:', _error);
      Alert.alert(
        'Error Creating Show',
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.dateInputWrapper}>
              ...
              <TextInput
                ...
                placeholder="Start date (_e.g., 2025-04-22)"
                placeholderTextColor="#999"
              />
            ...
            <View style={styles.dateInputWrapper}>
              ...
              <TextInput
                ...
                placeholder="End date (_e.g., 2025-04-24)"
                placeholderTextColor="#999"
              />
