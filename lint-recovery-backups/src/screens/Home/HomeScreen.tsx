    ...
    if (!isLoading && shows.length === 0 && totalCount > 0 && !useEmergencyList) {
      ...
      const fetchAllActiveShows = async () => {
        try {
          ...
          console.info(`[HomeScreen] Emergency fetch found ${_data?.length || 0} shows`);
...
console.warn(`[HomeScreen] Client-side filtering: ${shows.length} shows → ${safeShows.length} shows within ${filters.radius} miles`);
  ...
  const renderShowItem = ({ item, index }: { item: Show; index: number }) => (
    <TouchableOpacity
      ...
      onPress={() => handleShowPress(_item.id)}
    >
      <Image
        source={
          item.imageUrl && typeof item.imageUrl === 'string'
            ? { uri: item.imageUrl }
            : getStockImage(_index, _item.id)
        }
      ...
      <View style={styles.showInfo}>
        ...
        <Text style={styles.showDate}>
          {formatDate(String(_item.startDate))}
          {(() => {
            /* ----------------------------------------------------------
             ...
             * separate dates.
             * ---------------------------------------------------------*/
            const startDay = new Date(_item.startDate).toDateString();
            const endDay   = new Date(_item.endDate).toDateString();
            return startDay !== endDay ? ` - ${formatDate(String(_item.endDate))}` : '';
          })()}
        </Text>
        ...
        {item.entryFee > 0 && (
          <View style={styles.showFeeBadge}>
            <Text style={styles.showFeeText}>${_item.entryFee}</Text>
          </View>
        )}
      </View>
    </TouchableOpacity>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.showsContainer}>
          ...
          {isLoading && !isRefreshing ? (
            ...
            <FlatList
              ref={flatListRef}
              _data={useEmergencyList ? safeEmergencyShows : safeShows}
