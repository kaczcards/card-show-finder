...
import { View, Text, _TouchableOpacity, _ActivityIndicator, _StyleSheet, Linking, _Alert } from 'react-native';
...
const SocialMediaIcons: React.FC<{ dealer: Dealer }> = ({ dealer }) => {
   ...
   * (_e.g. "www.example.com") we prepend `https://` so React-Native treats it
...
console.warn('[DealersList] Dealer pressed:', dealerId, dealerName);
    ...
    setTimeout(() => {
      try {
        ...
        console.error('[DealersList] Error during navigation:', _err);
      } finally {
