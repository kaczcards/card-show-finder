...
import { _CardCategory, UserRole } from '../../types';
...
const DealerProfileScreen: React.FC = () => {
  ...
  const fetchDealerProfile = async () => {
    try {
      ...
      if (_data) {
        setDealerProfile(_data as unknown as DealerProfile);
        // Initialize form state with existing data
        setBusinessName(_data.business_name || '');
        setDescription(_data.description || '');
        setWebsiteUrl(_data.website_url || '');
        setInstagramUrl(_data.instagram_url || '');
        setTwitterUrl(_data.twitter_url || '');
        setFacebookUrl(_data.facebook_url || '');
        setBusinessEmail(_data.business_email || '');
        setBusinessPhone(_data.business_phone || '');
        setSelectedCategories(_data.categories || []);
        setOtherCategories(_data.other_categories || '');
      } else {
      ...
      setError(_err.message || 'Failed to load dealer profile');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      setError(_err.message || 'Failed to save dealer profile');
                ...
                <View style={styles.viewForm}>
                  {selectedCategories.length > 0 ? (
                    <View style={styles.categoriesList}>
                      {selectedCategories.map((category, _index) => (
