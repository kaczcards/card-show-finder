...
const ShowParticipationScreen: React.FC = () => {
  ...
  const loadDealerShows = useCallback(async () => {
    ...
    try {
      ...
      setDealerShows(_data || []);
    } catch (err: any) {
      setError(_err.message || 'Failed to load dealer shows');
    } finally {
      setIsLoading(false);
  ...
  const loadAvailableShows = useCallback(async () => {
    ...
    try {
      ...
      setAvailableShows(_data || []);
    } catch (err: any) {
      setError(_err.message || 'Failed to load available shows');
    } finally {
      setIsLoading(false);
      setIsRefreshing(false);
    }
  ...
  const togglePaymentMethod = (method: string) => {
    setFormData(prev => {
      ...
      if (_index > -1) {
        paymentMethods.splice(_index, 1);
      } else {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Alert.alert('Error', _err.message || 'Failed to register for show');
    }
  };
  ...
  const handleUpdateParticipation = async () => {
    ...
    try {
      ...
      Alert.alert('Error', _err.message || 'Failed to update booth information');
    }
  };
        ...
        { text: 'No', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', _err.message || 'Failed to cancel participation');
            }
          }
        }
      ]
        ...
        <View style={styles.showHeader}>
          ...
          <View style={styles.showDateContainer}>
            ...
            <Text style={styles.showDate}>
              {formatDateRange(item.startDate, _item.endDate)}
            </Text>
          </View>
        </View>
          ...
          <View style={styles.participationSection}>
            ...
            {item.participation.boothLocation && (
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{_item.participation.boothLocation}</Text>
              </View>
            )}
            
            {item.participation.cardTypes && Array.isArray(_item.participation.cardTypes) && item.participation.cardTypes.length > 0 && (
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Card Types:</Text>
                <Text style={styles.infoValue}>{_item.participation.cardTypes.join(', ')}</Text>
              </View>
            )}
            ...
            {item.participation.specialty && (
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{_item.participation.specialty}</Text>
              </View>
            )}
            ...
            {item.participation.priceRange && (
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>
                  {PRICE_RANGE_OPTIONS.find(opt => opt.value === _item.participation.priceRange)?.label || item.participation.priceRange}
                </Text>
              </View>
            )}
            ...
            <View style={styles.actionButtons}>
              ...
              {participationStatus !== 'cancelled' && (
                <TouchableOpacity 
                  ...
                  onPress={() => handleCancelParticipation(_item.participation.id)}
                >
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right']}>
      ...
      {isLoading && !isRefreshing ? (
        ...
        <>
          {activeTab === 'myShows' && (
            <FlatList
              _data={sortShowsByDate(dealerShows)}
          ...
          {activeTab === 'availableShows' && (
            <FlatList
              _data={sortShowsByDate(availableShows)}
              renderItem={renderShowItem}
              keyExtractor={(_item) => item.id}
              contentContainerStyle={styles.listContainer}
              refreshControl={
