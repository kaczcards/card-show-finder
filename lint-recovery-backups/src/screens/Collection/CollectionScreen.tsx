  ...
  const loadDealerInventory = async () => {
    ...
    try {
      ...
      const inventoryItem = data?.find(_item => 
        item.content && _item.content.startsWith(INVENTORY_PREFIX)
      );
      ...
      setInventoryError('An unexpected _error occurred. Please try again.');
      setInventoryContent('');
    } finally {
  ...
  const saveDealerInventory = async () => {
    ...
    try {
      ...
      if (inventoryId) {
        ...
        if (result._data) {
          setInventoryId(result._data.id);
        }
      }
      
      if (result._error) {
        console.error('Error saving inventory:', result._error);
        throw result.error;
      }
  ...
  const loadWantList = async () => {
    ...
    try {
      ...
      if (_error) {
        ...
        setWantListError(_error.message || 'Failed to load your want list');
        return;
      }
      ...
      console.error('Unexpected _error loading want list:', _err);
      setWantListError(err instanceof Error ? _err.message : 'An unexpected _error occurred');
    } finally {
  ...
  const loadUpcomingShows = async () => {
    ...
    try {
      ...
      if (_error) {
        ...
        setShowsError(typeof error === 'string' ? _error : 'Failed to load upcoming shows');
        setUpcomingShows([]);
      } else if (_data) {
        setUpcomingShows(_data as any[]); // Cast to any[]
      } else {
        setUpcomingShows([]);
      }
    } catch (_error) {
      console.error('Error in loadUpcomingShows:', _error);
      setShowsError(error instanceof Error ? error.message : 'An unexpected _error occurred');
      setUpcomingShows([]);
    } finally {
