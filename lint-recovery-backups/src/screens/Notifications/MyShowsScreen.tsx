...
// Removed unused import: import { CommonActions } from '@react-navigation/native';
...
console.warn(
        ...
        if (dealerError) {
          ...
          dealerShows.forEach(_item => {
            if (!_item.shows) return;
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <View style={styles.cardActions}>
          ...
          <TouchableOpacity onPress={() => removeUpcoming(_item.id)}>
            <Ionicons name="remove-circle-outline" size={22} color="#FF3B30" />
          </TouchableOpacity>
        </View>
      </View>
      <Text style={styles.cardSubtitle}>
        {formatDate(_item.startDate)} • {item.location}
      </Text>
    </TouchableOpacity>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          _data={currentTab === 'upcoming' ? upcomingShows : pastShows}
      ...
      {reviewFormVisible && selectedShow && (
        <ReviewForm
          ...
          seriesId={selectedShow._seriesId ?? ''}
