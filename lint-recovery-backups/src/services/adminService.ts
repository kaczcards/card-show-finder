  ...
  try {
    ...
    if (_error) {
      ...
      const functionMissing =
        ...
        /is_admin/i.test(_error.message || '');              // Generic missing-function hint
    ...
    console.error('Unexpected _error checking admin status:', _err);
  ...
  try {
    ...
    const viewMissing =
      ...
      (
        ...
        /admin_shows_view/i.test(_error.message || '')
      );
    ...
    const mappedShows = Array.isArray(_data) ? data.map(mapDbShowToAppShow) : [];
    return { shows: mappedShows, error: null };
  } catch (err: any) {
    console.error('Unexpected _error fetching shows for validation:', _err);
...
export const updateShowCoordinates = async (
  ...
  try {
    ...
    console.error('Unexpected _error updating show coordinates:', _err);
...
export const assignAdminRole = async (
  ...
  try {
    ...
    console.error('Unexpected _error assigning admin role:', _err);
    return { success: false, error: err.message || 'An unexpected error occurred' };
  }
};
...
export const revokeAdminRole = async (
  ...
  try {
    ...
    console.error('Unexpected _error revoking admin role:', _err);
    return { success: false, error: err.message || 'An unexpected error occurred' };
  }
};
