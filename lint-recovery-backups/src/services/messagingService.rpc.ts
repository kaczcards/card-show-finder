...
export const createDirectConversation = async (
  ...
  try {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to create conversation: ${_error.message}`);
    }
  ...
  try {
    ...
    const { data, error } = await supabase
      .rpc('create_group_conversation', {
        ...
        show_id: _showId || null
      });

    if (_error) {
      console.error('[messagingService/createGroupConversation] RPC error:', error);
      throw new Error(`Failed to create group conversation: ${_error.message}`);
    }
...
export const getConversationDetails = async (
  ...
  try {
    ...
    if (!data || _data._error) {
      console.error('[messagingService/getConversationDetails] Data error:', data?.error || 'No _data returned');
      return null;
    }
