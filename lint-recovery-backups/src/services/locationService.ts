...
const getZipFromCache = async (zipCode: string): Promise<ZipCodeData | null> => {
  try {
    ...
    console.warn('[locationService] Failed to read ZIP cache', _err);
    return null;
  }
};
...
const setZipCache = async (data: ZipCodeData): Promise<void> => {
  try {
    ...
    console.warn('[locationService] Failed to write ZIP cache', _err);
  }
};
...
export const getNearbyZipCodes = async (
  ...
  try {
    ...
    return (_data || []).map((item: NearbyZipRow) => item.zip_code);
  } catch (error: any) {
    console.error('Error getting nearby ZIP codes:', _error);
    throw new Error(_error.message || 'Failed to get nearby ZIP codes');
  }
};
 ...
 * @param coordinates Latitude and longitude
 * @returns Formatted string (_e.g., "37.7749,-122.4194")
 */
