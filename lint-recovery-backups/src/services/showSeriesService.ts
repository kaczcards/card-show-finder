...
console.warn('[showSeriesService.getAllShowSeries] Raw Supabase response:', {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to fetch show series: ${_error.message}`);
    }
  ...
  async getShowsInSeries(seriesId: string): Promise<Show[]> {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to fetch shows in series: ${_error.message}`);
    }
...
console.warn('[showSeriesService.getUnclaimedShows] Raw Supabase response:', {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to fetch unclaimed shows: ${_error.message}`);
    }
  ...
  extractSafeCoordinates(coordinatesData: any): { latitude: number; longitude: number } | undefined {
    ...
    try {
      ...
      console.error('[showSeriesService] Error extracting coordinates:', _error);
      return undefined;
    }
  },
  ...
  async getSeriesReviews(seriesId: string): Promise<Review[]> {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to fetch series reviews: ${_error.message}`);
    }
    ...
    return data.map(review => ({
      id: review.id,
      // Ensure the mandatory _showId is supplied (empty string fallback)
  ...
  async addSeriesReview(review: {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to add review: ${_error.message}`);
    }
  ...
  async respondToReview(reviewId: string, response: string): Promise<boolean> {
    ...
    if (_error) {
      ...
      throw new Error(`Failed to respond to review: ${_error.message}`);
    }
...
console.warn(
      ...
      console.error('[debugShowsTableColumns] Unexpected error:', _err);
    }
  },
  ...
  > & { _seriesId?: null }): Promise<{ success: boolean; show?: Show; error?: string }> {
    try {
      const payload = {
        ...showData,
        series_id: null,
      ...
      return { success: true, show: this.mapShowRow(_data) };
    } catch (_err) {
      console.error('Unexpected _error creating standalone show:', _err);
      return {
        success: false,
