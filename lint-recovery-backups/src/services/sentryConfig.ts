...
export const initSentry = (options: SentryConfigOptions = {}): void => {
  ...
  if (!SENTRY_DSN) {
    console.warn(
      ...
      'Add EXPO_PUBLIC_SENTRY_DSN to your .env file to enable _error reporting.'
    );
    return;
  }
...
export const captureException = (error: Error, context?: ScopeContext): void => {
  ...
  Sentry.Native.captureException(_error, context);
};
 ...
 * const transaction = startTransaction('loadUserData', '_data-loading');
 * try {
 *   await fetchUserData();
 *   transaction.setStatus('ok');
 * } catch (_error) {
 *   transaction.setStatus('_error');
