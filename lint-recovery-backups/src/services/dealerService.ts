...
export const getDealerShows = async (
  ...
  try {
    ...
    const transformedData = data.map(_item => {
      ...
      const participation = mapDbParticipationToAppParticipation(_item);
...
export const registerForShow = async (
  ...
  try {
    if (!userId || !participationData._showId) {
...
console.warn(
    ...
    const { data: existingReg, error: checkError } = await supabase
      ...
      .select('id')
      .eq('userid', userId)
      .eq('showid', participationData._showId)
      .maybeSingle();
...
export const getDealersForShow = async (
  ...
  try {
    ...
    const transformedData = participantsData.map(_item => {
      const participation = mapDbParticipationToAppParticipation(_item);
