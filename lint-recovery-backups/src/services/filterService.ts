 ...
 * between local storage (AsyncStorage) and server storage (Supabase).
 */
  ...
  try {
    ...
    const createdPreset: FilterPreset = {
      ...
      filters: {
        ...
        startDate: data.filters.startDate ? new Date(_data.filters.startDate) : null,
        endDate: data.filters.endDate ? new Date(_data.filters.endDate) : null,
      },
...
export const loadFilterPresetsFromSupabase = async (userId: string): Promise<FilterPreset[]> => {
  try {
    ...
    const presets: FilterPreset[] = data.map(_item => ({
      id: item.id,
      userId: item.user_id,
      name: item.name,
      filters: {
        ...
        startDate: item.filters.startDate ? new Date(_item.filters.startDate) : null,
        endDate: item.filters.endDate ? new Date(_item.filters.endDate) : null,
      },
...
export const syncFilters = async (userId: string): Promise<void> => {
  try {
    ...
    const mappedServerPresets: FilterPreset[] = serverPresets.map(_item => ({
      id: item.id,
      userId: item.user_id,
      name: item.name,
      filters: {
...
/**
 * Format a filter for display (_e.g., for filter chips or summaries)
 */
