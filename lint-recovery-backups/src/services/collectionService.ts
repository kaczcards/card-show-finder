...
export const getUserCards = async (userId: string): Promise<{ data: UserCard[] | null; error: any }> => {
  try {
    ...
    const transformedData = data
      ? (
          await Promise.all(_data.map((card) => mapSupabaseRecordToUserCard(card)))
...
export const addUserCard = async (
  ...
  try {
    ...
    if (_data?.imageurl) {
      const { data: url } = await storageService.getSignedUrl(_data.imageurl);
...
export const getUserWantList = async (
  ...
  try {
    ...
    if (error && _error.code !== 'PGRST116') { // PGRST116 is "no rows returned" error
      throw error;
    }
...
export const getDealersForShow = async (showId: string): Promise<{ 
  ...
  try {
    ...
    const dealers = data?.map(_item => _item.users) || [];
...
export const getSharedWantListsForDealer = async (
  ...
  try {
    ...
    const transformedData = data?.map(_item => ({
      id: item.id,
      ...
      wantList: item.wantLists ? {
        ...
        updatedAt: _item.wantLists.updatedat
      } : null
    }));
