...
class StorageService {
  ...
  async getSignedUrl(
    ...
    try {
      ...
      // Cache the URL
      this.cacheSignedUrl(cacheKey, _data.signedUrl, expiresIn);
      
      return { data: data.signedUrl, error: null };
    } catch (_error) {
      console.error('Error generating signed URL:', _error);
      return { data: null, error: error instanceof Error ? error : new Error(String(_error)) };
    }
  }
  ...
  async uploadImage(
    ...
    try {
      ...
      // any binary payload (_e.g. from `expo-image-picker`) is represented
      // as a `Blob`.
  ...
  async listUserImages(userId: string): Promise<StorageResult<string[]>> {
    try {
      ...
      // Extract file paths
      const paths = data
        .filter(_item => !_item.metadata?.isDir)
        .map(_item => `${userId}/${_item.name}`);
    ...
    try {
      ...
      await Promise.all(
        paths.map(async (path) => {
          ...
          if (_error) {
            console.warn(`Error getting signed URL for ${path}:`, _error);
            return;
          }
   ...
   * @param base64 - Base-64 encoded data (without _data-URI prefix)
   */
