 ...
 * is geo-aware – calling code (_e.g. HomeScreen) must supply them.
 */
...
export const getShows = async (filters: ShowFilters = {}): Promise<Show[]> => {
  try {
    ...
    console.info(
      `[showService] basic query returned ${((data && Array.isArray(_data)) ? data.length : 0)} show(s)`
    );
    ...
    throw new Error(_err.message ?? 'Failed to fetch shows');
  }
...
export const getPaginatedShows = async (
  ...
  try {
    ...
    console.error('[showService] Error in getPaginatedShows:', _err);
    return {
      data: [],
      pagination: {
        totalCount: 0,
...
const getDirectPaginatedShows = async (
  ...
  try {
    ...
    console.error('[showService] Error in getDirectPaginatedShows:', _err);
    return {
      data: [],
      pagination: {
        totalCount: 0,
...
const getAllActiveShowsFallback = async (
  ...
  try {
    ...
    if (dataError) {
      console.error('[showService] Error getting _data in emergency fallback:', dataError);
      throw dataError;
    }
    
    console.info(`[showService] Emergency getAllActiveShowsFallback found ${_data.length} shows (from ${count} total)`);
    ...
    console.error('[showService] Error in emergency fallback:', _err);
    return {
      data: [],
      pagination: {
        totalCount: 0,
...
export const claimShow = async (
  ...
  try {
    ...
    console.error('API _error in claimShow:', _err);
