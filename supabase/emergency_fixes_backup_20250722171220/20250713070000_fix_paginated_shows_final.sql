-- Migration: 20250713070000_fix_paginated_shows_final.sql
-- Description: Final fix for get_paginated_shows function addressing:
-- 1. GROUP BY clause error with s.coordinates
-- 2. Proper date filtering to show only upcoming shows
-- 3. Accurate location filtering using PostGIS

-- Drop the function to recreate it with the fixed version
DROP FUNCTION IF EXISTS public.get_paginated_shows;

-- Create the properly fixed function
