-- Migration: 20250203_fix_approve_function_v2.sql
-- Description: Fix type casting issue in approve_pending_show function
...
BEGIN
  ...
  -- Handle features - convert to JSONB or use empty array
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error handling features: %', SQLERRM;
    v_features := '[]'::jsonb;
  END;
  ...
  BEGIN
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error handling categories: %', SQLERRM;
    v_categories := ARRAY[]::text[];
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Failed to insert show into shows table',
      'detail', SQLERRM,
      'sqlstate', SQLSTATE
    );
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Warning: Could not update web_show_submissions: %', SQLERRM;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Warning: Could not log admin feedback: %', SQLERRM;
    -- Don't fail the whole operation if this fails
  END;
  ...
  IF v_pending_record.source_url IS NOT NULL THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Warning: Could not update source priority: %', SQLERRM;
      -- Don't fail the whole operation if this fails
    END;
  END IF;
...
COMMENT ON FUNCTION public.approve_pending_show IS 'Approves a pending show with robust error handling and proper type casting';

-- Done
DO $$
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Fixed approve_pending_show function v2';
  RAISE NOTICE 'Added proper JSONB type casting';
  RAISE NOTICE 'Added detailed error handling for each step';
  RAISE NOTICE '========================================';
END
$$;
