-- Migration: 20250819094500_update_show_details_rpc_include_booth_info.sql
-- Description: Updates the get_show_details_by_id function to include all booth details
-- for participating dealers, including show organizers, while remaining accessible to all users.
-- This ensures attendees can see complete booth information for all dealer types.

-- Drop the function if it already exists to ensure clean installation
DROP FUNCTION IF EXISTS public.get_show_details_by_id;

-- Create the improved function that includes complete booth details
