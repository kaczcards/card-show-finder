-- Migration: 20250713030000_fix_paginated_shows_group_by_error.sql
-- Description: Fixes GROUP BY error in get_paginated_shows function by ensuring all non-aggregated columns are included in the GROUP BY clause

-- Drop the function to recreate it with the fix
DROP FUNCTION IF EXISTS public.get_paginated_shows;

-- Create the function with a fixed GROUP BY clause
...
This version includes a GROUP BY clause to resolve the "column s.coordinates must appear in the GROUP BY clause" error.
