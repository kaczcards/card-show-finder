-- Migration: 20250713050000_fix_paginated_shows_bypass_date.sql
-- Description: Temporarily removes all date filtering from get_paginated_shows function
-- to diagnose if date filtering is causing the "No Shows Found" issue

-- Drop the function to recreate it with the diagnostic fix
DROP FUNCTION IF EXISTS public.get_paginated_shows;

-- Create the function with date filtering temporarily disabled
CREATE OR REPLACE FUNCTION public.get_paginated_shows(
  ...
  start_date timestamp with time zone DEFAULT current_date, -- Start of date range (ignored in this version)
  end_date timestamp with time zone DEFAULT (current_date + interval '30 days'), -- End of date range (ignored in this version)
...
BEGIN
  ...
  -- TEMPORARILY REMOVED DATE FILTER AND LOCATION FILTER FOR DIAGNOSTIC PURPOSES
  ...
  filtered_shows := jsonb_build_object(
    ...
    'diagnostic_info', jsonb_build_object(
      ...
      'original_start_date', get_paginated_shows.start_date,
      'original_end_date', get_paginated_shows.end_date
    )
  );
...
'DIAGNOSTIC VERSION: All date and location filtering temporarily removed.
Retrieves a paginated list of all active shows regardless of date or location.
...
Parameters:
  ...
  start_date - Start date for filtering shows (ignored in this diagnostic version)
  end_date - End date for filtering shows (ignored in this diagnostic version)
...
Returns:
  ...
  - diagnostic_info: Contains information about the current date and removed filters';
