-- Migration: 20250714060000_fix_show_details_table_error.sql
-- Description: Fixes the get_show_details_by_id function to not reference
-- the non-existent organizer_show_dealers table

-- Drop the function if it already exists to ensure clean installation
DROP FUNCTION IF EXISTS public.get_show_details_by_id;

-- Create the fixed function that only uses existing tables
...
BEGIN
  ...
  -- Only using the show_participants table (which exists)
  SELECT 
    jsonb_agg(
      jsonb_build_object(
        'id', p.id,
...
This fixed version only uses existing tables and does not reference the 
non-existent organizer_show_dealers table.';
