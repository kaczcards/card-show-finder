-- Migration: 20250203_fix_approve_function_v3.sql
-- Description: Fix approve function to handle coordinates and provide better debugging
-- Created: February 3, 2025

-- Drop and recreate the function with coordinates handling
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.shows (
      ...
      features,
      categories,
      coordinates,  -- Leave NULL for now, can be geocoded later
      created_at,
      updated_at
    ) VALUES (
      ...
      v_features,
      v_categories,
      NULL,  -- coordinates will be NULL initially
      now(),
      now()
    )
    RETURNING id INTO v_show_id;
  EXCEPTION WHEN OTHERS THEN
    -- Capture the full error detail
  ...
  -- Log the action (if admin_feedback table exists)
  BEGIN
  ...
  -- Update source priority score (if exists)
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Unexpected error in approve_pending_show',
    'sql_error', SQLERRM,
    'sql_state', SQLSTATE
  );
END;
...
COMMENT ON FUNCTION public.approve_pending_show IS 'Approves a pending show with coordinates handling and detailed error logging';
