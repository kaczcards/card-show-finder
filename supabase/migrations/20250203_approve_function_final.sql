-- Migration: 20250203_approve_function_final.sql
-- Description: Approve function without record type to avoid caching issues
-- Created: February 3, 2025

-- Drop all versions
DROP FUNCTION IF EXISTS public.approve_pending_show(UUID, TEXT);
DROP FUNCTION IF EXISTS public.approve_pending_show(UUID);
DROP FUNCTION IF EXISTS public.approve_pending_show;

-- Recreate WITHOUT using record type
CREATE FUNCTION public.approve_pending_show(
  p_pending_id UUID,
...
DECLARE
  ...
  v_source_url TEXT;
BEGIN
  -- Get the show data directly as JSONB (not as record type)
  SELECT 
    COALESCE(normalized_json, raw_payload)::JSONB,
    status,
    source_url
  INTO 
    v_show_data,
...
COMMENT ON FUNCTION public.approve_pending_show IS 'Approves a pending show (no record type to avoid cache issues)';
