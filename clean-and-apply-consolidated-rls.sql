-- ================================================================
-- CLEAN-AND-APPLY-CONSOLIDATED-RLS.SQL
-- ================================================================
-- This script:
-- 1. Drops ALL existing RLS policies
-- 2. Applies the consolidated RLS policies
-- 3. Uses dynamic SQL to ensure clean application
-- ================================================================
...
BEGIN
    RAISE NOTICE 'Starting to drop all existing RLS policies...';
    
    -- Loop through all existing policies in public schema
    FOR policy_rec IN 
        SELECT 
            policyname, 
            tablename 
    ...
    LOOP
        -- Construct DROP POLICY statement
        drop_sql := format('DROP POLICY IF EXISTS %I ON %I', 
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING 'Error dropping policy % on table %: %', 
                policy_rec.policyname, policy_rec.tablename, SQLERRM;
        END;
    END LOOP;
    
    RAISE NOTICE 'Successfully dropped % RLS policies', policy_count;
END $$;
