-- Migration: FIXED_infinite_recursion_sql.sql
...
-- This version fixes the syntax error by removing "IF NOT EXISTS" and adding proper error handling.
...
-- Using DO block with exception handling to make it idempotent (safe to run multiple times)
DO $$
BEGIN
  -- First try to drop the policy if it exists (silently continue if it doesn't)
  BEGIN
  ...
  -- Now create the policy (will succeed whether or not it existed before)
...
EXCEPTION
  WHEN duplicate_object THEN
    RAISE NOTICE 'Policy "show_participants_select_self" already exists, skipping creation';
END $$;
